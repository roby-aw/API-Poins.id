...
func (repo *PosgresRepository) GetOrderEmoney(emoney *user.TransactionBank) (*user.TransactionBank, error) {
	var err error
	...
	var datafind user.TransactionBank

	repo.db.Where("ID_Transaction = ?", hasil).First(datafind)
	if datafind.ID_Transaction != "" {
		inthasil = inthasil + 1
		strhasil = strconv.Itoa(inthasil)
	}
	emoney.ID_Transaction = "EM" + strhasil
	defer repo.db.Create(&emoney)
