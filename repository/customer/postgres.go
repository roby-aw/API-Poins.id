...
func (repo *PosgresRepository) UpdateCustomer(Data *customerBusiness.UpdateCustomer) (*customerBusiness.UpdateCustomer, error) {
	...
	err := repo.db.Model(&customerBusiness.Customers{}).Where("ID = ?", Data.ID).Updates(customerBusiness.Customers{Email: Data.Email, Fullname: Data.Fullname, No_hp: Data.No_hp}).Error
	if err != nil {
		return nil, err
	}
	return Data, nil
