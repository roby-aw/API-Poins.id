...
func (repo *PosgresRepository) InsertStore(store *customerBusiness.RegisterStore) (*customerBusiness.RegisterStore, error) {
	...
	err := repo.db.Model(&repository.Customer{}).Create(&customerBusiness.Store{Email: store.Email, Password: string(hash), Store: store.Store, Alamat: store.Alamat}).Error
	if err != nil {
		return nil, err
	}
	return store, nil
